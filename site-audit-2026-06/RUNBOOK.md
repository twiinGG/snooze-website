...
**Changelog**
- 2026-06-09 (positioning + resume): Sally positioning standard added (`docs/brand/SALLY-POSITIONING.md`, wired into root + app AGENTS.md): "internationally certified sleep consultant and former paediatric nurse" (never current/registered); "Access with the Snooze Membership" not "Lifetime access". Repo-wide sweep applied it (~80 nurse-framing + 75 lifetime-access fixes across ~40 files; verbatim testimonials, guest refs, and founding-member "lifetime price lock" left). Founding-member page left whole (date-stale, needs rebuild). Live glossary meta description corrected via Kajabi MCP (verified live). **Website + blog SEO push blocked in this environment:** website-page SEO has no MCP write (legacy theme) and the authenticated Kajabi admin session (agent-browser) repeatedly drops on the Save POST (two clean attempts, zero writes landed, live pages unchanged). Use `ADMIN-PASTE-CHECKLIST.md` §1/§2 for manual paste; all recommended copy in `page_seo` + the checklist now follows the positioning standard.
