...
| CUSTOMIZER block Ace (section-block code editor in the Encore customizer) | `ace.edit(...).setValue()` + REAL editor keystroke (`ed.insert(" "); ed.remove("left")`) + TRUSTED click on the enabled Save | **Yes — SUPERSEDED 2026-07-02 evening (census §0, KAJABI-CODE-SURFACE-MAP.md).** The 2026-07-01 "No" verdict used DOM-event dirty-nudges, which do not enable Save; the editor-API keystroke does. Proven byte-exact on the homepage's 50KB block. (Historic note: the July 1 failure is why the homepage originally shipped via a runtime enhancer.) |
...
**RESOLVED 2026-07-02: both paths work.** The earlier "block-save is blocked" conclusion conflated the customizer path (which needed the editor-API keystroke, now proven) with the builder path (which always worked). Every section-block write path is available to the page wave; pick per the census's per-page "Method proven to write it" column.
