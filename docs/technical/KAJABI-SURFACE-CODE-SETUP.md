...
- **Where code lives:**
  ...
  - **Shared JS:** `settings-js-input` -> `global/js/theme-custom-code.js` (one-file overwrite). Kajabi loads this field on every website page. Its current behaviour is guarded to `#home-page` after the 2026-07-27 split and exits safely elsewhere. GTM, currency toggle and `[data-checkout]` helpers need website and landing-page coverage, so they live once in Header Page Scripts, not here. Do not paste `snooze-globals.js` or `currency-toggle.js`.
...
- **Checkout Tracking Code** (Settings → Checkout, at `/admin/settings/checkout`): site-wide Header and Footer tracking fields placed in every checkout page when that offer's `inject_header_tracking_code` / `inject_footer_tracking_code` flags are true. Confirmed live 2026-07-27 (admin screenshot). Operator index: [`PASTE-MAP.md`](../../kajabi-deployment/PASTE-MAP.md) rows A4/A5. Canonical sources:
  - Header: [`checkout-header-tracking.html`](../../kajabi-deployment/global/html/checkout-header-tracking.html) only (**already live; do not re-paste to sync**). To add Advanced Matching later, inline [`meta-advanced-matching.js`](../../kajabi-deployment/global/js/meta-advanced-matching.js) into that file in git, then whole-field overwrite once. Never append a second file at paste time.
  - Footer: [`kajabi-checkout-tracking.js`](../../kajabi-deployment/global/js/kajabi-checkout-tracking.js) (live empty as of 2026-07-27).
...
| Checkout Tracking Code (header) | Site-wide, every checkout when inject_header is true | n/a (scripts) | Settings → Checkout → Header tracking code | YES - single file `checkout-header-tracking.html` (inline AM into it before shipping) |
