...
6. **Cloudflare/auth:** stored profile cookies die between sessions; automated navigation to `/login` triggers a WAF block. Recovery = a HUMAN headed login in a fresh named profile (`agent-browser open https://app.kajabi.com/login --headed --profile <dir> --session <name>`), then agents drive that named session. One named session per workstream; never share a session between concurrent agent conversations.
