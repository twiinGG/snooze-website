...
- **Checkout Tracking Code** (Settings → Checkout, at `/admin/settings/checkout`): site-wide Header and Footer tracking fields placed in every checkout page when that offer's `inject_header_tracking_code` / `inject_footer_tracking_code` flags are true. Confirmed live 2026-07-27 (admin screenshot). Operator index: [`PASTE-MAP.md`](../../kajabi-deployment/PASTE-MAP.md) rows A4/A5. Canonical sources:
  - Header: [`checkout-header-tracking.html`](../../kajabi-deployment/global/html/checkout-header-tracking.html) (**already live; do not re-paste to sync**). Optional: append [`meta-advanced-matching.js`](../../kajabi-deployment/global/js/meta-advanced-matching.js) (strip its comment header) after the GTM/Stape block.
