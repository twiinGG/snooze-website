...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    // Only fill placeholder [data-checkout] hrefs (e.g. home #pricing).
    // Never clobber a real /offers/<slug> URL; that hijacks course, consult,
    // and Camp Snooze CTAs onto membership checkout (live bug 2026-07-27).
    ...
    targets.forEach((el) => {
      ...
      const href = keepOfferUrl
        ? currentHref
        : (isPlaceholderHref(currentHref) ? (globalCheckoutUrl || fallbackUrl) : currentHref);
      if (el.tagName.toLowerCase() === 'a' && href !== currentHref) {
        el.setAttribute('href', href);
      }
      if (el.classList.contains('feature-box') && el.tagName.toLowerCase() !== 'a') {
        el.addEventListener('click', () => {
          window.location.href = href || globalCheckoutUrl || fallbackUrl;
        });
...
<!-- ===== SNOOZE CURRENCY TOGGLE — bundled; do not paste separately ===== -->
<script>
(function() {
  'use strict';
...
<!-- ===== THEME HOME HELPERS — bundled; do not paste separately ===== -->
<script>
document.addEventListener('DOMContentLoaded', function() {
