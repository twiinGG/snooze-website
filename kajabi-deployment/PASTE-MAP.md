...
| A6 | Website theme → **Navigation custom-code block** | website theme navigation section | [`global/html/navigation.html`](./global/html/navigation.html) | Website pages | Repo points Membership to `/snooze-membership`, uses the trial checkout and removes the expired launch banner | **No current write.** Membership is live and the ratified native `Start Here` destination is `/snooze-membership`; re-verify and obtain exact-mutation approval before any future paste |
