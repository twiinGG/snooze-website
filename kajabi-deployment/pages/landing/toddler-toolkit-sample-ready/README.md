# Toddler Toolkit sample ready (post-purchase page)

**Kajabi surface:** landing page, id to be assigned at create. Slug `toddler-toolkit-sample-ready`.
**Status:** built in repo, **not yet pasted and not yet live**. Copy is propose-only until Sally approves it.

Post-purchase destination for offer `4HQjFJGC`. Once WS-003 workstream 3 lands, the offer is also the free resource on `/toddler-sleep-help`.
...
|---|---|
| `README.md` | This file |
| `toddler-toolkit-sample-ready.html` | Page body. Pastes into the landing page's Custom Code block |
| `toddler-toolkit-sample-ready.css` | Self-contained stylesheet. Pastes into the landing page's own Custom CSS field |
...
`toddler-toolkit-sample-ready.css` is derived from [`../catnapping-guide-ready/catnapping-guide-ready.css`](../catnapping-guide-ready/catnapping-guide-ready.css), with three renames and two deletions:

| Change | Reason |
|---|---|
| `#catnapping-guide-ready-page` becomes `#toddler-sample-ready-page` | Page wrapper id |
| `.snooze-container` becomes `.tsr-wrap` | WS-003 decision 6, see below |
| `.snooze-section` becomes `.tsr-band` | WS-003 decision 6, see below |
| `.cgr-*` becomes `.tsr-*` | Page prefix |
...
Every page-scoped class here is prefixed `tsr-` and contains none of the three. The canonical footer classes (`.snooze-footer-clean`, `.sf-*`) are the deliberate exception: they are a sync source shared with every other page and must not be renamed here.
...
1. **Create** the landing page on site `2148291177`. Title and slug `toddler-toolkit-sample-ready`.
2. **Custom CSS.** Settings, Custom CSS, paste `toddler-toolkit-sample-ready.css` whole.
3. **Page body.** Paste `toddler-toolkit-sample-ready.html` into the Custom Code block. Fragment only. Do **not** paste `<!DOCTYPE>`, `<html>`, `<head>` or `<body>`; a full document makes Kajabi serve two `<title>` tags.
...
6. **Only then** set offer `4HQjFJGC` (`2150846925`) post-purchase to `landing_page` with this page's id.
...
The page makes no claim that emails are coming, because no email sequence is bound to this offer. `list_sequences` on site `2148291177` returns 38 sequences and none targets `Toddler Toolkit - Sample Access`. The automations toolset is not enabled on this account, so no automation binding can be read either way.
...
- [ ] View source: wrapper is `<div id="toddler-sample-ready-page">`, never `<body id>`
...
- [ ] **`getComputedStyle` check.** Expected: `h1` in `"Playfair Display", serif`, `.tsr-band` padding-top `72px` (`56px` under 768px), `a.btn` background `rgb(244, 51, 87)` with 50px radius, `.tsr-trial-note` 14.4px in `rgb(100, 116, 139)`
...
- [ ] Offer `4HQjFJGC` post-purchase points at this page's id, confirmed by `get_offer`
...
| Offer slug | `4HQjFJGC` |
| Offer id | `2150846925` |
| Offer title | Toddler Toolkit - Sample Access |
| Granted product id | `2149272894` |
| Asset URL on the page | `https://www.joinsnooze.com/products/88d5fab2-212c-4d84-8fcd-8aa5c4d9ce0c` |
...
This offer is currently linked from nowhere on the site (register D6). Wiring it onto `/toddler-sleep-help` depends on this page being live first.
