...
- **There is no camp picker.** It was removed on 2026-08-25 with the single-intake urgency change: the funnel now sells one camp at a time, so a buyer arrives here holding the camp they paid for and the page's job is to confirm it, not to reopen the choice. A quiet line under the button tells anyone who needs a different camp to reply to their confirmation email before confirming. `pickDefault` still resolves which camp is being confirmed, and still never offers a `full` or `closed` one.
...
- **That setting is ON on all six camp offers as of August 21, 2026**, applied on Kade's decision and verified per offer by reloading its Settings tab and reading `offer_collect_password_checkbox` back: `2150884129`, `2150946767`, `2150947919`, `2151264520`, `2151114090`, `2151134284`. If it is ever switched off, every brand-new buyer sees the signin state instead of the confirm button. Check this before concluding the page is broken; it is a toggle on six offers, not a code change.
