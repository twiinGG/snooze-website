(function () {
  ...
  var root = document.getElementById('sn-trial-thank-you-page');
  if (!root || root.getAttribute('data-sn-bound') === 'true') return;

  var paths = Array.prototype.slice.call(root.querySelectorAll('.sn-ty-path'));
  var result = root.querySelector('#sn-ty-result');
  var title = root.querySelector('#sn-ty-result-title');
  var action = root.querySelector('#sn-ty-result-action');
  var resultLink = root.querySelector('#sn-ty-result-link');
  var villageLink = root.querySelector('#sn-ty-village-link');
  ...
  function emit(eventName, details) {
    var payload = Object.assign({ event: eventName, surface: 'trial_thank_you' }, details || {});
  ...
  if (villageLink) {
    villageLink.addEventListener('click', function () {
      emit('village_cta_click', { placement: 'trial_thank_you_age_path' });
    });
  }
