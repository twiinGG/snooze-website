...
const html = fs.readFileSync(path.join(base, 'thank-you-page.html'), 'utf8');
const css = fs.readFileSync(path.join(base, 'thank-you-page.css'), 'utf8');
const js = fs.readFileSync(path.join(base, 'thank-you-page.js'), 'utf8');
...
assert('HTML uses the landing-page wrapper', html.includes('<div id="sn-trial-thank-you-page">'));
...
assert('HTML exposes four direct age-path links', (html.match(/class="sn-ty-path"/g) || []).length === 4);
...
assert('HTML contains no stale Community Library link', !html.includes('/products/communities/v2/snooze/library'));
...
assert('CSS is scoped to the landing-page wrapper', css.includes('#sn-trial-thank-you-page'));
assert('CSS enforces hidden result state', /\.sn-ty-result\[hidden\][\s\S]*display:\s*none\s*!important/.test(css));
assert('CSS provides a one-column mobile path layout', /@media \(max-width: 640px\)[\s\S]*\.sn-ty-paths[\s\S]*grid-template-columns:\s*1fr/.test(css));
assert('CSS includes reduced-motion handling', css.includes('@media (prefers-reduced-motion: reduce)'));
assert('JS scopes initialization to the page wrapper', js.includes("document.getElementById('sn-trial-thank-you-page')"));
...
const selectorMap = {
  '.sn-ty-path': [firstPath, secondPath],
  '[data-app-platform]': [appLink],
  '#sn-ty-result': result,
  '#sn-ty-result-title': title,
  '#sn-ty-result-action': action,
  '#sn-ty-result-link': resultLink,
  '#sn-ty-village-link': villageLink
};

const root = element();
root.querySelectorAll = selector => selectorMap[selector] || [];
root.querySelector = selector => selectorMap[selector] || null;

const context = {
  document: { getElementById: id => id === 'sn-trial-thank-you-page' ? root : null },
...
assert('selection includes the thank-you surface', context.window.dataLayer[0].surface === 'trial_thank_you');
...
assert('Village click emits the expected placement', villageEvent.event === 'village_cta_click' && villageEvent.placement === 'trial_thank_you_age_path');

appLink.listeners.click();
const appEvent = context.window.dataLayer[context.window.dataLayer.length - 1];
assert('app click emits the platform and placement', appEvent.event === 'app_download_click' && appEvent.platform === 'ios' && appEvent.placement === 'trial_thank_you');
