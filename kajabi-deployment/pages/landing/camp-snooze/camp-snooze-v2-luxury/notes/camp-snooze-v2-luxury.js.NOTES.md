...
> **The extractor overwrites this file, it does not merge.** `extract-comments.mjs` regenerates the
> notes from the comments still present in the source, so running it after the source has already
> been stripped writes a file containing only the newest batch. Every earlier note is lost, and this
> file is their only copy. It happened on 2026-08-25 and the notes below were restored from git.
> Before running the extractor, check what this file already holds.
...
## Notes added 2026-08-25, hero next-camp banner

Line anchors below are from the commit that added `renderHeroNext`. The notes above predate it and
their anchors sit about fifteen lines earlier than their current position in the source.

## Line 93

```
// The hero used to say nothing about when the next camp runs, so the only date on the page sat
```
...
```
// below the fold in the capacity card. This mirrors the chosen cohort's start date into the hero
```
...
```
// from the same feed, so the two can never disagree. No date in the feed means the hero stays
```
...
```
// hidden rather than showing an empty or stale line.
```
